...
hostnames = open('routers.txt').read().split()
...
for hostname in hostnames:
    ...
    if response == 0:
        ...
        for int in all_ints:
            if int.re_search_children("NWSCJD"):
                for line in int.ioscfg:
                    if "interface" in line:
                        ...
                        time.sleep(3)
                        remote_connection.send(' description ///WIRELESS///\n')
                        remote_connection.send('switchport access vlan 501\n')
                        remote_connection.send('switchport mode access\n')
                        remote_connection.send('spanning-tree bpduguard enable\n')
                        remote_connection.send('spanning-tree portfast\n')
